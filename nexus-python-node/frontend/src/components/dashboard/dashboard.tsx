...
const SectorHeatmapLazy = lazy(() => import('./sector-heatmap').then(m => ({ default: m.SectorHeatmap })));
function SectorHeatmap(props: any) {
  return (
    <Suspense fallback={<div style={{ height: 180, display: 'flex', alignItems: 'center', justifyContent: 'center', color: U.textMute, fontSize: 11 }}>Loading sectors...</div>}>
      <SectorHeatmapLazy {...props} />
    </Suspense>
  );
}
