...
# Load symbols from stock-symbols.json
_SYMBOLS_CACHE: List[Dict[str, str]] = []

def get_all_symbols() -> List[Dict[str, str]]:
    global _SYMBOLS_CACHE
    if not _SYMBOLS_CACHE:
        # Load local stock-symbols.json
        base_dir = os.path.dirname(os.path.abspath(__file__))
        json_path = os.path.join(base_dir, "symbols", "stock-symbols.json")
        try:
            if os.path.exists(json_path):
                with open(json_path, "r", encoding="utf-8") as f:
                    _SYMBOLS_CACHE = json.load(f)
            else:
                _SYMBOLS_CACHE = SP500_TOP100
        except Exception:
            _SYMBOLS_CACHE = SP500_TOP100
    return _SYMBOLS_CACHE

def search_local_symbols(query: str) -> List[Dict[str, str]]:
    q_upper = query.upper()
    ...
    for s in symbols:
        sym = s.get("sym", "")
        name = s.get("name", "")
        if sym.startswith(q_upper) or q_upper in name.upper():
            matches.append({"sym": sym, "name": name})
    return matches[:20]
